...
class Cell(object):
    ...
    miss_cell = set_color('Т', Color.miss)
...
# выстрел
        if input_type == "shot":
            ...
            if self.is_ai:
                ...
                user_input = input().upper().replace("Т", "")
...
if __name__ == '__main__':
    ...
    players.append(Player(name='Противник', is_ai=True, auto_ship=True, skill=1))
    ...
    while True:
        ...
        if game.status == 'in game':
            ...
            Game.clear_screen()
            game.current_player.message.append("Ждем координаты выстрела: ")
            game.draw()
